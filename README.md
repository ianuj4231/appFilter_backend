...
3. Redis installation : https://redis.io/docs/latest/operate/oss_and_stack/install/install-redis/



4. ![image](https://github.com/user-attachments/assets/71adacee-b604-459d-b02e-a88b8b6be815)

5.   ![image](https://github.com/user-attachments/assets/c000388d-4122-4bf4-a5df-da057f087f9e)
 
6. ![image](https://github.com/user-attachments/assets/ba5d5b76-5d92-49ad-acea-7e8513df04e6)



7. type cmd in top-search bar

   
   
![image](https://github.com/user-attachments/assets/0c098ffb-3640-4af6-9c72-5bfa1dfdfbc8)



9. type command: `redis-server.exe`  and press enter


    
![image](https://github.com/user-attachments/assets/78e83562-9de7-4616-9aa6-f32d6ea23402)


11. This turns on the redis cache server:

    
![image](https://github.com/user-attachments/assets/131e7682-f101-4505-978b-96324d39c7eb)
...
1. Get the second page of products, sorted by rating in descending order:
   `http://localhost:8080/api/v1/fashion/filter?page=1&sortby=rating&sortdirection=desc`

2. Filter products with size `S` and color `red`:
   `http://localhost:8080/api/v1/fashion/filter?size=S&color=red`

3. Filter products with size `XXL`, color `green`, and rating `3`:
   `http://localhost:8080/api/v1/fashion/filter?size=XXL&color=green&rating=3`

4. Filter products by brand `h&m` (URL-encoded `&` as `%26`), sorted by price:
   `http://localhost:8080/api/v1/fashion/filter?brand=h%26m&sortby=price`

5. Get the second page of products sorted by price:
   `http://localhost:8080/api/v1/fashion/filter?sortby=price&page=1`

6. Get the fourth page of products sorted by price:
   `http://localhost:8080/api/v1/fashion/filter?sortby=price&page=3`

7. Filter products with size `L`:
   `http://localhost:8080/api/v1/fashion/filter?size=L`

8. Filter products with size `M` and rating `4`:
